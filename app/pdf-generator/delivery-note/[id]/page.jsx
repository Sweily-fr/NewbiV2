...
import DeliveryNotePreview from "@/app/dashboard/outils/bons-de-livraison/components/DeliveryNotePreview";
...
export default function DeliveryNotePDFGeneratorPage() {
  ...
  useEffect(() => {
    async function fetchAndGenerate() {
      try {
        console.log("🔍 Récupération bon de livraison:", params.id);

        const response = await fetch(`/api/delivery-notes/data/${params.id}`);
  ...
  return (
    <div className={isPrint ? "bg-white" : "bg-white min-h-screen p-4"}>
      ...
      >
        {data && (
          <DeliveryNotePreview data={data} forPDF={true} />
        )}
      </div>
    </div>
  );
