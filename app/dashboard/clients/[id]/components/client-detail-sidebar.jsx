...
export default function ClientDetailSidebar({
  ...
  const invoiceStats = useMemo(() => {
    const normalTotal = clientInvoices.reduce(
      (sum, inv) => sum + (inv.finalTotalTTC || inv.totalTTC || 0),
      0,
    );
    const normalPending = clientInvoices
      .filter((inv) => inv.status === "PENDING")
      .reduce((sum, inv) => sum + (inv.finalTotalTTC || inv.totalTTC || 0), 0);
    const importedTotal = importedInvoices.reduce(
      (sum, inv) => sum + (inv.totalTTC || 0),
      0,
    );
    const importedPending = importedInvoices
      .filter(
        (inv) => inv.status === "VALIDATED" || inv.status === "PENDING_REVIEW",
      )
      .reduce((sum, inv) => sum + (inv.totalTTC || 0), 0);
    return {
      count: clientInvoices.length + importedInvoices.length,
      total: normalTotal + importedTotal,
      pending: normalPending + importedPending,
    };
  }, [clientInvoices, importedInvoices]);
