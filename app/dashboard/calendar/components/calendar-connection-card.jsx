...
export function CalendarConnectionCard({ connection, onRefresh }) {
  ...
  const [updateSelectedCalendars, { loading: toggling }] = useMutation(
      ...
      <div className="group rounded-xl border border-border/60 bg-card transition-all hover:border-border hover:shadow-sm">
        ...
        <div className="p-3">
          ...
          <div className="flex items-center gap-2.5">
            <div
              className={cn(
                "flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-border/50 shadow-sm",
                info.iconBg,
              )}
            >
              {info.icon}
          ...
          <div className="mt-2 flex items-center justify-between">
            <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
              {allCalendars.length > 0 && (
                <span>
                  {enabledCalendars.length}/{allCalendars.length} calendrier
                  {allCalendars.length > 1 ? "s" : ""}
                </span>
              )}
          ...
          {connection.lastSyncError && (
            <p
              className="mt-1.5 text-[11px] text-red-500 truncate"
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  {cal.color ? (
                    <div
                      className={cn(
                        "h-2.5 w-2.5 rounded-full shrink-0 transition-opacity",
                    ...
                    <div
                      className={cn(
                        "h-2.5 w-2.5 rounded-full shrink-0 bg-muted-foreground/40 transition-opacity",
                        !cal.enabled && "opacity-30",
                      )}
                    />
                  )}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tous les evenements importes depuis ce calendrier seront supprimes
              de Newbi. Cette action est irreversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
