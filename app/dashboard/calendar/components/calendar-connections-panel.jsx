...
const providers = [
  ...
  },
  {
    ...
    iconBg:
      "bg-white dark:bg-zinc-800 border-violet-100 dark:border-violet-900/50",
  },
  {
    id: "apple",
    name: "Apple Calendar",
...
export function CalendarConnectionsPanel() {
  ...
  const handleCalendarConnect = useCallback(
    async (provider) => {
      if (!isReady) {
        toast.error(
          "Session non disponible. Veuillez rafraichir la page et reessayer.",
        );
        return;
      }
      ...
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        ...
        <SheetContent className="w-full sm:w-[460px] sm:max-w-[460px] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto mt-4 space-y-5 px-4">
                  ...
                  {limitReached && (
                    <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-900/50 dark:bg-amber-950/20 p-3">
                      <p className="text-xs text-amber-800 dark:text-amber-300">
                        Limite atteinte ({connections.length}/{calendarLimit}{" "}
                        calendrier{calendarLimit > 1 ? "s" : ""}).{" "}
            ...
            {/* All connected state */}
            {availableProviders.length === 0 &&
              connections.length >= providers.length && (
                <>
                  <Separator />
