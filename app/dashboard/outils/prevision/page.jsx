    ...
    >
      <div className="flex flex-col min-h-full">
        ...
        <div className="pt-4 sm:pt-6 mb-6 px-4 sm:px-6">
                ...
                {maxPeriod > 0 &&
                  monthSpan(customStart.slice(0, 7), customEnd.slice(0, 7)) >
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        Horizon limité à {maxPeriod} mois sur votre plan — la
                        fin est ramenée en conséquence.
                      </TooltipContent>
                    </Tooltip>
                  )}
              </div>
          ...
          {/* Rappel : un scénario est un calque sur Base, rien n'y remonte. */}
          {activeScenarioId && (
            <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 rounded-lg border border-[#5b4fff]/20 bg-[#5b4fff]/5 px-3 py-2 text-xs text-muted-foreground">
              <GitBranch size={13} className="text-[#5b4fff] shrink-0" />
              <span>
                Scénario{" "}
                <span className="font-medium text-foreground">
                  «&nbsp;{activeScenario?.name || "…"}&nbsp;»
                </span>{" "}
                : les prévisions ajoutées, masquées ou supprimées ici ne
                modifient pas le scénario Base.
              </span>
              <button
                type="button"
