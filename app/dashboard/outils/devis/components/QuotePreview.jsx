...
export default function QuotePreview({ data = {}, className = '', status }) {
  ...
  // Debug: Afficher la structure des données companyInfo
  console.log('QuotePreview - data.companyInfo:', data.companyInfo);
  console.log('QuotePreview - data.companyInfo?.legal:', data.companyInfo?.legal);
  console.log('QuotePreview - data.companyInfo?.siret:', data.companyInfo?.siret);
  console.log('QuotePreview - data.companyInfo?.vatNumber:', data.companyInfo?.vatNumber);
  ...
  return (
    <div className={className}>
      ...
      >
        {/* Header avec logo newbi. et titre DEVIS */}
        <div className="flex justify-between items-start mb-8">
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: '10px', color: '#666' }}>
              <p style={{ margin: '2px 0' }}>N° {data.number ? `${data.prefix || ''}${data.number}` : 'DRAFT-' + Math.random().toString(36).substr(2, 9)}</p>
              <p style={{ margin: '2px 0' }}>Date d'émission: {formatDate(data.issueDate) || formatDate(new Date())}</p>
              <p style={{ margin: '2px 0' }}>Date de validité: {formatDate(data.validityDate) || formatDate(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000))}</p>
              {data.executionDate && (
                <p style={{ margin: '2px 0' }}>Date d'exécution: {formatDate(data.executionDate)}</p>
              )}
              {data.purchaseOrderNumber && (
                <p style={{ margin: '2px 0' }}>Référence: {data.purchaseOrderNumber}</p>
              )}
            </div>
          </div>
        </div>
            ...
            <div style={{ width: '200px' }}>
              ...
              {data.customFields && data.customFields.length > 0 && (
                data.customFields.map((field, index) => {
                  ...
                  console.log('QuotePreview - Custom field:', field);
                  console.log('QuotePreview - field.label:', field.label);
                  console.log('QuotePreview - field.value:', field.value);
                  console.log('QuotePreview - field.name:', field.name);
                  console.log('QuotePreview - field keys:', Object.keys(field));
              ...
              <div style={{ 
                ...
                {data.bankDetails?.iban && (
                  <div>
                    ...
                    <span style={{ fontFamily: 'monospace' }}>{data.bankDetails.iban}</span>
                  </div>
                )}
                {data.bankDetails?.bic && (
                  <div>
                    <strong>BIC/SWIFT:</strong><br />
                    <span style={{ fontFamily: 'monospace' }}>{data.bankDetails.bic}</span>
                  </div>
                )}
