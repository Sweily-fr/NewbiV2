...
export function usePurchaseOrderEditor({
  ...
  const form = useForm({
    ...
    resolver: (values) => {
      ...
      if (values.issueDate) {
        ...
        // Validation de la date de livraison (optionnelle)
  ...
  // Re-valider quand le numéro de bon de commande change (avec debounce)
  useEffect(() => {
    // Ne pas valider si le formulaire n'est pas encore initialisé
    if (!isFormInitialized) return;
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (number === "000000") {
        setValidationErrors((prevErrors) => ({
          ...prevErrors,
          purchaseOrderNumber: {
            message: 'Le numéro de bon de commande ne peut pas être "000000"',
            canEdit: true,
          },
        }));
        return;
      ...
      // Validation simple via usePurchaseOrderNumber
      const validation = validatePurchaseOrderNumber(number);
  ...
  // Re-valider quand la date de livraison change (avec debounce)
  useEffect(() => {
    // Ne pas valider si le formulaire n'est pas encore initialisé
    if (!isFormInitialized) return;
    ...
    const timeoutId = setTimeout(() => {
      setValidationErrors((prevErrors) => {
        if (watchedDeliveryDate && watchedIssueDate) {
          const issueDate = new Date(watchedIssueDate);
          const deliveryDate = new Date(watchedDeliveryDate);
  ...
  const validateStep1 = useCallback(() => {
    ...
    // Vérifier la date de livraison (optionnelle, mais si fournie doit être après issueDate)
  ...
  const handleSave = useCallback(
    async (isAutoSave = false) => {
      try {
        ...
        // Validation des informations du bon de commande (dates)
        const orderInfoErrors = [];

        // Vérifier la date d'émission
        if (!currentFormData.issueDate) {
          orderInfoErrors.push("date d'émission manquante");
        } else {
          ...
          if (currentFormData.deliveryDate) {
    ...
    async (formDataOverride) => {
      try {
        ...
        if (existingPurchaseOrder?.id) {
          result = await updatePurchaseOrder(existingPurchaseOrder.id, input);
        } else {
          result = await createPurchaseOrder(input);
        }
        ...
        if (result?.id) {
          ...
          return {
            ...
            shouldRedirect: true,
            redirectUrl: "/dashboard/outils/bons-commande",
...
function transformPurchaseOrderToFormData(purchaseOrder) {
  ...
  if (
    purchaseOrder.deliveryDate !== undefined &&
  ...
  return {
      ...
      ? {
          ...
          ...(purchaseOrder.client.type === "COMPANY"
            ? {
                ...
                vatNumber: purchaseOrder.client.vatNumber,
                contactFirstName: purchaseOrder.client.contactFirstName,
                contactLastName: purchaseOrder.client.contactLastName,
              }
            : {
                firstName: purchaseOrder.client.firstName,
                lastName: purchaseOrder.client.lastName,
              }),
        }
      : null,
    ...
    companyInfo: {
      ...
      siren: purchaseOrder.companyInfo?.siren || "",
      ...
      bankDetails: purchaseOrder.companyInfo?.bankDetails
        ? {
            bankName: purchaseOrder.companyInfo.bankDetails.bankName || "",
            iban: purchaseOrder.companyInfo.bankDetails.iban || "",
            bic: purchaseOrder.companyInfo.bankDetails.bic || "",
          }
        : null,
    },
    ...
    // Récupérer showBankDetails depuis le bon de commande existant, par défaut false
    ...
    userBankDetails: purchaseOrder.companyInfo?.bankDetails
      ? {
          iban: purchaseOrder.companyInfo.bankDetails.iban || "",
          bic: purchaseOrder.companyInfo.bankDetails.bic || "",
          bankName: purchaseOrder.companyInfo.bankDetails.bankName || "",
    ...
    // Paramètres d'apparence depuis le bon de commande existant
...
function transformFormDataToInput(
  ...
  // Traitement de la date de livraison (optionnelle)
  ...
  const finalDeliveryDate = deliveryDate ? formatFinalDate(deliveryDate) : null;

  // Si la date de livraison est fournie, vérifier qu'elle est postérieure à la date d'émission
  if (finalDeliveryDate) {
    const issueDateObj = new Date(finalIssueDate);
    const deliveryDateObj = new Date(finalDeliveryDate);

    if (deliveryDateObj < issueDateObj) {
      // Ajuster la date de livraison pour être au moins égale à la date d'émission
      // Ne pas envoyer une date de livraison invalide
    }
  }
