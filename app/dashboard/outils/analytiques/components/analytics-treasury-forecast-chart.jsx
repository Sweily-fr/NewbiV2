    ...
    <div className="rounded-lg border bg-background p-3 shadow-sm text-sm min-w-[200px]">
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="font-medium">
            {formatCurrency(data.forecastIncome)}
          </span>
        </div>
        <div className="flex items-center justify-between gap-4">
          <span className="flex items-center gap-2">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="font-medium">
            {formatCurrency(data.forecastExpense)}
          </span>
        </div>
...
export function AnalyticsTreasuryForecastChart({ forecastData, loading }) {
  ...
  const chartConfig = useMemo(
    () => ({
      actualIncome: { label: "Revenus réels", color: chartColors.success },
      forecastIncome: {
        label: "Revenus prévus",
        color: chartColors.successLight,
      },
      actualExpense: { label: "Dépenses réelles", color: chartColors.danger },
      forecastExpense: {
        label: "Dépenses prévues",
        color: chartColors.dangerLight,
      },
      closingBalance: { label: "Solde", color: remap("#3b82f6") },
    }),
    [chartColors, remap],
  );
        ...
        <ChartContainer config={chartConfig} className="h-[300px] w-full">
          ...
          >
            ...
            <Line
              ...
              stroke={remap("#3b82f6")}
              strokeWidth={2.5}
              dot={{ r: 4, fill: remap("#3b82f6") }}
              activeDot={{ r: 6, fill: remap("#3b82f6") }}
            />
          </ComposedChart>
        </ChartContainer>
      </CardContent>
