...
export function GmailConnectionDialog({ open, onOpenChange }) {
  ...
  const handleConnect = async () => {
    try {
      ...
      // Récupérer le JWT pour l'appel authentifié
      ...
      const authorizeUrl = `${API_URL}/gmail-connect/authorize?scanPeriodMonths=${scanPeriod}`;

      // Appel authentifié au backend pour obtenir l'URL OAuth Google
      ...
      if (!res.ok) {
        console.error(`Gmail authorize failed [${res.status}]:`, responseText);
        let errorMsg;
        try {
          errorMsg = JSON.parse(responseText).error;
        } catch {}
        throw new Error(errorMsg || `Erreur ${res.status} lors de la connexion Gmail`);
      }

      const { authUrl } = JSON.parse(responseText);
      window.location.href = authUrl;
    } catch (err) {
      console.error("Erreur connexion Gmail:", err);
      const { toast } = await import("@/src/components/ui/sonner");
      toast.error(err.message || "Impossible de lancer la connexion Gmail");
      setConnecting(false);
    }
  };
