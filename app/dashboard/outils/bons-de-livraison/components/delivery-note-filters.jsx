...
export default function DeliveryNoteFilters({
  statusFilter,
  setStatusFilter,
  clientFilter,
  setClientFilter,
  ...
  const uniqueClients = useMemo(() => {
    ...
    deliveryNotes.forEach((dn) => {
      if (dn.client?.name) {
        clientNames.add(dn.client.name);
      }
    });
  ...
  const statuses = Object.keys(DELIVERY_NOTE_STATUS_LABELS);
            ...
            {statuses.map((status) => (
              ...
              >
                <Checkbox
                  ...
                  className="mr-2 pointer-events-none"
                />
                <span>{DELIVERY_NOTE_STATUS_LABELS[status]}</span>
              </div>
            ))}
          </DropdownMenuSubContent>
        </DropdownMenuSub>
