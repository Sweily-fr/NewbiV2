...
const DynamicSocialLogo = ({ logoType, color, size = 24 }) => {
  ...
  useEffect(() => {
    const generateLogo = async () => {
      try {
        ...
        console.log(`🎨 Génération logo ${logoType} couleur ${color}`);
  ...
  if (error || !imageUrl) {
    const SVG_FALLBACKS = {
      facebook: (
        <svg width={size} height={size} viewBox="0 0 50 50" style={{ display: 'block' }}>
          <path fill={color} d="M41,4H9C6.24,4,4,6.24,4,9v32c0,2.76,2.24,5,5,5h32c2.76,0,5-2.24,5-5V9C46,6.24,43.76,4,41,4z M37,19h-2c-2.14,0-3,0.5-3,2 v3h5l-1,5h-4v15h-5V29h-4v-5h4v-3c0-4,2-7,6-7c2.9,0,4,1,4,1V19z"/>
        </svg>
      ),
      linkedin: (
    ...
    return SVG_FALLBACKS[logoType] || (
      <div style={{ width: size, height: size, backgroundColor: '#e5e7eb', borderRadius: '2px' }} />
    );
  }
