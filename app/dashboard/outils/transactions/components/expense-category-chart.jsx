...
export function ExpenseCategoryChart({
  ...
  const { data, loading: queryLoading } = useQuery(GET_CATEGORY_AGGREGATION, {
    variables: {
      workspaceId,
      type: "expense",
      period,
  ...
  const chartData = useMemo(() => {
    ...
    // Couleur par nom de catégorie : identique au camembert « Dépenses par
    // catégorie » des Analytiques, quel que soit le rang de la part.
    const colors = assignExpenseColors(categories.map((c) => c.name));
  ...
  // Total from backend
  const totalAmount = data?.dashboardCategoryAggregation?.total || 0;
