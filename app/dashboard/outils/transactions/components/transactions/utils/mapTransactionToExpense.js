...
export const mapTransactionToExpense = (tx) => ({
  ...
  // N↔N : "a une facture liée" = array non vide (facture client, facture
  // d'achat OU facture client importée — le justificatif d'une facture
  // d'achat liée vaut justification).
  hasReceipt:
    (Array.isArray(tx.receiptFiles) && tx.receiptFiles.length > 0) ||
    (tx.linkedInvoices?.length || 0) > 0 ||
    (tx.linkedPurchaseInvoices?.length || 0) > 0 ||
    (tx.linkedImportedInvoices?.length || 0) > 0,
