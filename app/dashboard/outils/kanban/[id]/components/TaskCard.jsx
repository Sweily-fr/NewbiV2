...
import { Calendar, MoreVertical, Edit, Trash2, GripVertical, Flag, CheckSquare, Tag } from 'lucide-react';
...
import { getPriorityColor, getPriorityIcon, formatDateRelative } from '../../../../../../src/utils/kanbanHelpers';
...
export function TaskCard({ task, onEdit, onDelete }) {
  ...
  const handleClick = (e) => {
    ...
    const interactiveElements = ['BUTTON', 'A', 'INPUT', 'SELECT', 'TEXTAREA', 'DIV[role="menuitem"]'];
  ...
  return (
    <>
      <div
        ...
        {...listeners}
        onClick={handleClick}
        className={`bg-card text-card-foreground rounded-lg border border-border p-3 mb-3 shadow-sm hover:shadow-md transition-all duration-200 ease-out hover:bg-accent/50 will-change-transform ${isSortableDragging ? 'opacity-50' : ''} cursor-pointer`}
      >
      {/* En-tête de la carte */}
      ...
      {task.tags && task.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2">
          {task.tags.map((tag, index) => {
            ...
            const tagName = typeof tag === 'object' ? tag.name || tag.title || 'Tag' : tag;
            return (
              <Badge
                key={index}
                variant="outline"
        ...
        <div className="flex items-center gap-1">
          {task.dueDate && (
            <>
              ...
              <span>{new Date(task.dueDate).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
            </>
          )}
        </div>
