...
/**
 * Ligne Gantt à montage paresseux. Le conteneur de 45px est toujours présent
 * (l'alignement vertical reste correct), mais le contenu lourd (avatars,
 * popovers, tooltips, barres) n'est rendu que lorsque la ligne approche la zone
 * visible — puis le reste au scroll. `children` est une fonction pour ne même
 * pas construire le JSX hors écran.
 */
function LazyGanttRow({
  rootRef,
  className,
  style,
  onClick,
  ...
  const [ref, isVisible] = useLazyVisible({ rootRef, rootMargin: "600px 0px" });
  return (
    <div
      ref={ref}
      className={className}
...
export function KanbanGanttView({
  ...
  const getTaskBarStyle = (task) => {
    ...
    if (
      ...
      resizingTask.currentDelta
    ) {
      const originalStartDate = task.startDate
        ? startOfDay(parseISO(task.startDate))
        : null;
      const originalEndDate = task.dueDate
        ? startOfDay(parseISO(task.dueDate))
        : null;
  ...
  const handleResizeMove = (e) => {
    ...
    if (
      resizingTask.side === "left" &&
      resizingTask.originalStartDate &&
      resizingTask.originalDueDate
    ) {
      const newStartDate = addDays(
        parseISO(resizingTask.originalStartDate),
        daysDelta,
      );
    ...
    if (
      resizingTask.side === "right" &&
      resizingTask.originalDueDate &&
      resizingTask.originalStartDate
    ) {
      const newDueDate = addDays(
        parseISO(resizingTask.originalDueDate),
        daysDelta,
      );
      ...
      while (element && element !== timeline) {
        if (
          ...
          (element.classList.contains("cursor-pointer") ||
            element.hasAttribute("data-task-bar"))
        ) {
                                ...
                                >
                                  ...
                                  {task.startDate && (
                                    <div
                                      ...
                                      style={{
                                        backgroundColor: task.column.color,
                                        left: "-10px",
                                      }}
                                      onMouseDown={(e) =>
                                        handleResizeStart(e, task, "left")
                                      }
                                    />
                                  )}
                                  ...
                                  {task.dueDate && (
                                    <div
                                      ...
                                      onMouseDown={(e) =>
                                        handleResizeStart(e, task, "right")
                                      }
                                    />
                                  )}
