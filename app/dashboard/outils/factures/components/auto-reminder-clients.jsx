...
export default function AutoReminderClients() {
  ...
  console.log("👥 [AutoReminderClients] Clients chargés:", clients?.map(c => ({ id: c.id, name: c.name })));
  ...
  const someClientsSelected = excludedClientIds.length > 0 && excludedClientIds.length < (clients?.length || 0);
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-3">
          Sélectionnez les clients qui recevront les relances automatiques. 
          Les clients non cochés ne recevront pas de relance.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 p-3 bg-[#5b50ff]/10 border border-[#5b50ff]/20 rounded-lg">
        ...
        <p className="text-sm text-[#5b50ff]">
          <span className="font-medium">{activeClientsCount}</span> client(s) sur{" "}
          <span className="font-medium">{clients?.length || 0}</span> recevront les relances
        </p>
      </div>
            ...
            return (
              ...
              >
                <Checkbox
                  ...
                  onCheckedChange={() => {
                    if (isExcluded) {
                      setValue("excludedClientIds", excludedClientIds.filter(id => id !== client.id));
                    } else {
                      setValue("excludedClientIds", [...excludedClientIds, client.id]);
                    }
                  }}
                />
