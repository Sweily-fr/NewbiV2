...
// Skeleton partagé des éditeurs de facture et d'avoir.
// Reprend la structure réelle de ModernInvoiceEditor / ModernCreditNoteEditor
// (overlay plein écran, grille 2 colonnes : formulaire à gauche, aperçu PDF à
// droite) pour que loading.jsx, les guards et les fallbacks Suspense
// affichent tous le même visuel pendant l'attente.

// Grille 2 colonnes sans le wrapper plein écran, pour pouvoir l'insérer
// aussi bien dans l'overlay que dans la page détail.
function EditorPanelsSkeleton() {
  return (
...
// Éditeur plein écran : création (/new), édition (/editer) et avoirs
export function InvoiceEditorSkeleton() {
  return (
    <div className="fixed inset-0 z-40 flex flex-col overflow-hidden bg-background">
      <EditorPanelsSkeleton />
    </div>
  );
}

// Page détail /factures/[id] : header (retour + titre + actions) puis éditeur
