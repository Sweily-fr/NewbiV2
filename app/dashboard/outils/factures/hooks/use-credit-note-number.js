...
export const useCreditNoteNumber = () => {
  ...
  useEffect(() => {
    if (data?.creditNotes?.creditNotes) {
      ...
      const finalizedCreditNotes = data.creditNotes.creditNotes.filter(
        (creditNote) => {
          const isFinalized = creditNote.status === "CREATED";
          const hasNumber =
            creditNote.number && creditNote.number.trim() !== "";
  ...
  const getNextCreditNoteNumber = () => {
    ...
    const lastNumber =
      typeof lastCreditNoteNumber === "number"
        ? lastCreditNoteNumber
        : parseInt(lastCreditNoteNumber, 10);
  ...
  const validateCreditNoteNumber = (number) => {
    ...
    // le dernier numéro utilisé et le numéro attendu. Les avoirs n'ont pas de
    // séquence par préfixe, donc pas de préfixe dans le message.
    const last = `Le dernier avoir est le ${String(lastNum).padStart(4, "0")}`;
    const expected = String(lastNum + 1).padStart(4, "0");
  ...
  const hasExistingCreditNotes = () => {
    return (
      ...
      lastCreditNoteNumber !== undefined &&
