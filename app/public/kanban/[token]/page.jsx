...
function AuthModal({ isOpen, token, shareLinkName, onSuccess, onBanned }) {
  ...
  const [authenticate, { loading: authenticating }] = useMutation(
    AUTHENTICATE_INVITED_USER,
    {
      onCompleted: (data) => {
    ...
    <Dialog open={isOpen} onOpenChange={() => {}}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            {step === "email" &&
              "Veuillez renseigner votre adresse email pour accéder à ce tableau."}
            {step === "password" &&
              "Entrez votre mot de passe pour vous connecter."}
            {step === "create_account" &&
              "Configurez votre profil pour accéder au tableau."}
          </DialogDescription>
        </DialogHeader>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.data?.requestAccess?.success) {
        ...
        toast.error(
          result.data?.requestAccess?.message || "Erreur lors de la demande",
        );
      }
    } catch (error) {
          ...
          {requestSent ? (
            <div className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Votre demande d'accès a été envoyée au propriétaire du
                  tableau. Vous serez notifié par email une fois qu'elle sera
                  traitée.
                </p>
              </div>
              <div className="pt-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="p-4 bg-muted/50 rounded-lg border border-border">
                <p className="text-sm text-muted-foreground mb-3">
                  Vous pouvez demander un nouvel accès au propriétaire du
                  tableau.
                </p>
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      if (result.data?.uploadVisitorImage?.success) {
        ...
        toast.error(
          result.data?.uploadVisitorImage?.message || "Erreur lors de l'upload",
        );
        setImagePreview(visitorProfile?.image || null);
      }
    } catch (error) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Personnalisez votre profil pour les commentaires.
          </DialogDescription>
        </DialogHeader>
            ...
            <Popover open={showDescPopover} onOpenChange={setShowDescPopover}>
              <PopoverTrigger asChild>
                <div
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    setShowDescPopover(!showDescPopover);
                  }}
                >
              ...
              <PopoverContent className="w-80" side="top">
                <div className="space-y-2">
                  ...
                  <div
                    className="text-sm text-muted-foreground whitespace-pre-wrap break-words prose prose-sm max-w-none"
  ...
  return (
    <>
      ...
      <div
        ...
        className={`rounded-xl p-1.5 sm:p-2 min-w-[240px] max-w-[240px] sm:min-w-[300px] sm:max-w-[300px] flex flex-col flex-shrink-0 ${isCollapsed ? "max-w-[80px] min-w-[80px]" : ""}`}
      >
        <div
          className={`flex items-center justify-between gap-2 px-2 ${isCollapsed ? "" : "mb-2 sm:mb-3"}`}
        >
        ...
        {!isCollapsed && (
          <div
            className="kanban-column-scroll p-2 pb-4 rounded-lg overflow-y-auto"
            style={{ maxHeight: maxHeight }}
          >
            <div className="flex flex-col gap-2 sm:gap-3">
              {tasks.map((task) => (
                <PublicTaskCard key={task.id} task={task} onEdit={onEditTask} />
              ))}
            </div>
          </div>
        )}
...
function PublicTaskActivity({
  ...
  const [uploadExternalCommentImage] = useMutation(
    UPLOAD_EXTERNAL_COMMENT_IMAGE,
  );
  ...
  const handleAddComment = async () => {
    if ((!newComment.trim() && pendingImages.length === 0) || !canComment)
      return;
  ...
  const getDisplayName = (item) => {
    ...
    if (!item.isExternal && item.userId && boardMembers.length > 0) {
      const member = boardMembers.find(
        (m) => m.id === item.userId || m.userId === item.userId,
      );
  ...
  const getActivityDisplay = (activity) => {
    ...
    if (
      (activity.type === "assigned" || activity.type === "unassigned") &&
        ...
        if (memberNames.length > 0) {
          text =
            activity.type === "assigned"
              ? `a assigné ${memberNames.join(", ")}`
              : `a désassigné ${memberNames.join(", ")}`;
        }
      }
    }
  ...
  const renderCommentItem = (comment) => (
    ...
    >
      <div className="flex gap-3">
        <UserAvatar
          src={getDisplayImage(comment)}
          name={getDisplayName(comment)}
          colorKey={comment.userEmail}
          size="sm"
                ...
                <>
                  <div className="flex items-center gap-1">
                    <div
                      className="w-2 h-2 rounded-full flex-shrink-0"
                      style={{
                        backgroundColor: display.moveDetails.from.color,
                      }}
                    />
            ...
            {activities.length === 0 ? (
              ...
              <>
                ...
                {(showAllActivities ? activities : activities.slice(-3)).map(
                  (activity, index) => renderActivityItem(activity, index),
                )}
              </>
            )}
          </TabsContent>
          ...
          <div className="pb-3 pl-3 pr-3 pt-1 space-y-2 flex-shrink-0">
              ...
              <Textarea
                ...
                placeholder={
                  isDragOver
                    ...
                    : "Ajouter un commentaire... (glissez-déposez des images)"
                }
            ...
            <div className="flex justify-between items-center">
              ...
              <Button
                ...
                disabled={
                  (!newComment.trim() && pendingImages.length === 0) ||
                  addingComment ||
                  uploadingImages
                }
              >
            ...
            {!isCollapsed && (
              <div className="w-full overflow-x-auto scrollbar-hide">
                <div className="w-max min-w-full">
                  ...
                  <div
                    className="grid px-2 py-2 text-xs font-normal text-muted-foreground/70 tracking-wide border-b border-border/60"
                    style={{
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex-1 w-0 flex items-center gap-1 min-w-0">
                              <p className="text-sm truncate font-normal text-foreground/90 group-hover:text-foreground">
                                {task.title}
                              </p>
                              {task.description && (
                                <Popover>
...
function PublicGanttView({
  ...
  const getTaskBarStyle = (task) => {
    ...
    const isVisible =
      isWithinInterval(taskStart, { start: firstDay, end: lastDay }) ||
  ...
  const getMemberInfo = (memberId) => {
    ...
    return (
      boardMembers?.find((m) => m.id === memberId || m.userId === memberId) || {
        id: memberId,
        name: memberId,
      }
    );
    ...
    <div className="flex flex-col h-[calc(100vh-10rem)] md:h-[calc(100vh-10rem)] bg-background">
      ...
      <div className="flex-1 overflow-hidden border-t border-border">
        ...
        <div className="flex border-b border-border">
          {/* Header gauche */}
          <div
            className="w-32 sm:w-48 md:w-72 border-r border-border bg-background px-2 sm:px-3 md:px-4 flex items-start pt-2 flex-shrink-0"
            style={{ height: "74px" }}
          >
              ...
              <div className="flex">
                {daysToDisplay.map((day, index) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`text-sm font-normal ${isToday ? "text-primary" : "text-foreground"}`}
                      >
        ...
        <div
          className="grid grid-cols-[128px_1fr] sm:grid-cols-[192px_1fr] md:grid-cols-[288px_1fr] h-[calc(100%-74px)] overflow-y-auto"
          ref={leftColumnRef}
        >
          {/* Colonne des tâches */}
          <div
            ...
            >
              ...
              <div className="relative min-h-full">
                ...
                <div className="absolute inset-0">
                  ...
                  {Array.from({
                    length: Math.max(0, 20 - allTasks.length),
                  }).map((_, index) => (
...
function PublicTaskModal({
  ...
  const getCreatorName = () => {
    ...
    const creator = boardMembers.find(
      (m) =>
        String(m.id) === String(task.userId) ||
        String(m.userId) === String(task.userId),
    );
                        ...
                        {assignedMembersInfo.slice(0, 5).map((member, idx) => (
                          ...
                          >
                            <UserAvatar
                              ...
                              style={{
                                zIndex: assignedMembersInfo.length - idx,
                              }}
                            />
                          </div>
                        ))}
              ...
              {task.timeTracking && (
                <div className="mt-6 space-y-3">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    >
                      <div
                        className={`w-2 h-2 rounded-full ${task.timeTracking.isRunning ? "bg-white" : "bg-gray-600"}`}
                      />
                    </div>
                      ...
                      task.timeTracking.startedBy && (
                        <div className="flex items-center gap-2 px-2 py-1 text-xs text-muted-foreground flex-shrink-0">
                          ...
                          <span className="font-medium text-foreground">
                            {task.timeTracking.startedBy.userName}
                          </span>
                        </div>
                      )}
                ...
                <div className="space-y-3 mt-6">
                  ...
                  <div className="space-y-2 pl-6">
                    {task.checklist.map((item, index) => (
                      ...
                      >
                        <div
                          className={`w-4 h-4 rounded border flex items-center justify-center ${item.completed ? "bg-primary border-primary" : "border-input"}`}
                        >
                          {item.completed && (
                  ...
                  {task.updatedAt && task.updatedAt !== task.createdAt && (
                    <div className="flex items-center gap-1.5">
                      <span className="text-muted-foreground/70">
                        • Modifié le {formatCreatedDate(task.updatedAt)}
                      </span>
                    </div>
                  )}
                </div>
          ...
          >
            ...
            >
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
                ...
                {currentColumn && (
                  <div className="flex items-center gap-4">
                    <Label className="text-sm font-normal w-24 flex-shrink-0">
                      Status
                    </Label>
                    <div
                ...
                {task.timeTracking && (
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${task.timeTracking.isRunning ? "bg-white" : "bg-gray-600"}`}
                        />
                      </div>
...
export default function PublicKanbanPage({ params }) {
  ...
  const [getPublicBoard, { loading: loadingBoard }] =
    useLazyQuery(GET_PUBLIC_BOARD);
  const [updateVisitorProfile, { loading: updatingProfile }] = useMutation(
    UPDATE_VISITOR_PROFILE,
  );
  ...
  useSubscription(PUBLIC_TASK_UPDATED_SUBSCRIPTION, {
    ...
    onData: ({ data }) => {
      ...
      console.log(
        "📡 [Public] Mise à jour temps réel reçue:",
        payload.type,
        payload.taskId,
      );
  ...
  useSubscription(ACCESS_REVOKED_SUBSCRIPTION, {
    ...
    onData: ({ data }) => {
      ...
      if (payload) {
        ...
        toast.error(
          "Votre accès a été révoqué par le propriétaire du tableau.",
        );
      ...
      try {
        ...
        if (storedSession) {
          ...
          if (session.email) {
            ...
            const result = await getPublicBoard({
              variables: { token, email: session.email },
            });
            if (result.data?.getPublicBoard?.success) {
              ...
              const visitor = result.data.getPublicBoard.share?.visitors?.find(
                (v) => v.email === session.email.toLowerCase(),
              );
              if (visitor) {
                setVisitorProfile({
    ...
    try {
      ...
      if (result.data?.getPublicBoard?.success) {
        ...
        if (result.data?.getPublicBoard?.isBanned) {
          ...
          setAccessError(
            result.data?.getPublicBoard?.message || "Accès refusé",
          );
        }
      }
    } catch (error) {
  ...
  const filteredTasksByColumn = useMemo(() => {
    ...
    Object.keys(tasksByColumn).forEach((colId) => {
      result[colId] = tasksByColumn[colId].filter(
        (task) =>
    ...
    return (
      <BannedAccessPage
        ...
        onAccessApproved={async () => {
          ...
          try {
            const result = await getPublicBoard({
              variables: { token, email: visitorEmail },
            });
            if (result.data?.getPublicBoard?.success) {
              setBoardData(result.data.getPublicBoard.board);
              setPermissions(result.data.getPublicBoard.share?.permissions);
              ...
              const visitor = result.data.getPublicBoard.share?.visitors?.find(
                (v) => v.email === visitorEmail.toLowerCase(),
              );
              if (visitor) {
                setVisitorProfile({
                  firstName: visitor.firstName,
            ...
            <div className="flex items-center justify-between gap-3 py-3 border-b px-4 sm:px-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2 pl-2 border-l border-border">
                  <UserAvatar
                    src={visitorProfile?.image}
                    name={visitorProfile?.name || visitorEmail}
                    colorKey={visitorEmail}
                    size="sm"
                  />
                  <span className="text-sm text-muted-foreground hidden sm:inline">
                    {visitorProfile?.name || visitorEmail}
                  </span>
                </div>
              </div>
            </div>
