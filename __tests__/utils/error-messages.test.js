...
describe("getErrorMessage", () => {
  ...
  it("detects authentication errors from message", () => {
    ...
    expect(getErrorMessage("authentication failed")).toBe(
      ERROR_MESSAGES.AUTH.INVALID_CREDENTIALS,
    );
  });
  ...
  it("relaie tel quel les messages de numérotation, numéro intercalé compris", () => {
    const messages = [
      ...
      'Le numéro de facture "0012" est déjà utilisé',
      "Ce numéro de devis est déjà utilisé",
      "Le numéro de facture F-082026 0001 existe déjà",
      'La dernière facture avec le préfixe "F-082026" est la 0012. Il y a un trou dans la séquence : le prochain numéro doit être 0013.',
    ];

    for (const message of messages) {
      // Le message de l'API est déjà rédigé pour l'utilisateur : il ne doit pas
      // être remplacé par un message générique, même avec un code d'erreur.
  ...
  it("detects network errors from message", () => {
    ...
    expect(getErrorMessage("service unavailable")).toBe(
      ERROR_MESSAGES.NETWORK.SERVER_UNAVAILABLE,
    );
  });
  ...
  it("handles error codes - NOT_FOUND with context", () => {
    ...
    expect(getErrorMessage(error, "client")).toBe(
      ERROR_MESSAGES.CLIENT.NOT_FOUND,
    );
    expect(getErrorMessage(error, "invoice")).toBe(
      ERROR_MESSAGES.INVOICE.NOT_FOUND,
    );
    expect(getErrorMessage(error, "quote")).toBe(
      ERROR_MESSAGES.QUOTE.NOT_FOUND,
    );
  });
  ...
  it("handles DUPLICATE_KEY code with context", () => {
    ...
    expect(getErrorMessage(error, "client")).toBe(
      ERROR_MESSAGES.CLIENT.ALREADY_EXISTS,
    );
    expect(getErrorMessage(error, "invoice")).toBe(
      ERROR_MESSAGES.INVOICE.DUPLICATE_NUMBER,
    );
  });

  it("returns contextual default message for unknown errors", () => {
    expect(getErrorMessage("some random error", "invoice")).toBe(
      ERROR_MESSAGES.INVOICE.CREATION_FAILED,
    );
    expect(getErrorMessage("some random error", "client")).toBe(
      ERROR_MESSAGES.CLIENT.CREATION_FAILED,
    );
    expect(getErrorMessage("some random error", "auth")).toBe(
      ERROR_MESSAGES.AUTH.INVALID_CREDENTIALS,
    );
    expect(getErrorMessage("some random error", "generic")).toBe(
      ERROR_MESSAGES.GENERIC.UNKNOWN_ERROR,
    );
  });

  it("detects file upload errors", () => {
    expect(getErrorMessage("upload failed")).toBe(
      ERROR_MESSAGES.FILE.UPLOAD_FAILED,
    );
    expect(getErrorMessage("upload too large file")).toBe(
      ERROR_MESSAGES.FILE.TOO_LARGE,
    );
  });

  it("detects SIRET validation errors", () => {
    expect(getErrorMessage("siret invalide")).toBe(
      ERROR_MESSAGES.VALIDATION.INVALID_SIRET,
    );
  });

  it('detects "already exists" messages with context', () => {
    expect(getErrorMessage("cet email existe déjà", "client")).toBe(
      ERROR_MESSAGES.CLIENT.ALREADY_EXISTS,
    );
    expect(getErrorMessage("already exists", "invoice")).toBe(
      ERROR_MESSAGES.INVOICE.DUPLICATE_NUMBER,
    );
  });
});
...
describe("isCriticalError", () => {
  ...
  it("returns true for UNAUTHENTICATED error code", () => {
    expect(isCriticalError({ code: "UNAUTHENTICATED", message: "" })).toBe(
      true,
    );
  });
  ...
  it("returns true for SESSION_EXPIRED error code", () => {
    expect(isCriticalError({ code: "SESSION_EXPIRED", message: "" })).toBe(
      true,
    );
  });
...
describe("requiresUserAction", () => {
  ...
  it("returns true for COMPANY_INFO_INCOMPLETE code", () => {
    expect(
      requiresUserAction({ code: "COMPANY_INFO_INCOMPLETE", message: "" }),
    ).toBe(true);
  });

  it("returns true for EMAIL_NOT_VERIFIED code", () => {
    expect(
      requiresUserAction({ code: "EMAIL_NOT_VERIFIED", message: "" }),
    ).toBe(true);
  });

  it("returns true for messages matching action patterns", () => {
    expect(requiresUserAction("Veuillez compléter les informations")).toBe(
      true,
    );
