...
describe("refreshPrefixDate", () => {
  const may2026 = new Date(2026, 4, 1); // May 1, 2026

  it("refreshes YYYYMM at end (year first)", () => {
    expect(refreshPrefixDate("F-202604", may2026)).toBe("F-202605");
  });

  it("refreshes MMYYYY at end (month first)", () => {
    expect(refreshPrefixDate("F-042026", may2026)).toBe("F-052026");
  });

  it("refreshes YYYY-MM with dash separator", () => {
    expect(refreshPrefixDate("F-2026-04", may2026)).toBe("F-2026-05");
  });

  it("refreshes MM-YYYY with dash separator", () => {
    expect(refreshPrefixDate("F-04-2026", may2026)).toBe("F-05-2026");
  });

  it("refreshes YYYY-only at end", () => {
    expect(refreshPrefixDate("F-2025", may2026)).toBe("F-2026");
  });

  it("crosses year boundary (December → January)", () => {
    const jan2026 = new Date(2026, 0, 15);
    expect(refreshPrefixDate("F-202512", jan2026)).toBe("F-202601");
  });

  it("returns prefix unchanged when no date pattern found", () => {
    expect(refreshPrefixDate("FACTURE-", may2026)).toBe("FACTURE-");
    expect(refreshPrefixDate("ABC-XYZ", may2026)).toBe("ABC-XYZ");
  });

  it("returns prefix unchanged when month is invalid (>12)", () => {
    expect(refreshPrefixDate("F-22-2026", may2026)).toBe("F-22-2026");
  });

  it("handles null/empty input", () => {
    expect(refreshPrefixDate(null, may2026)).toBeNull();
    expect(refreshPrefixDate("", may2026)).toBe("");
    expect(refreshPrefixDate(undefined, may2026)).toBeUndefined();
  });
  ...
  it("uses current date when no date argument provided", () => {
    ...
    expect(refreshPrefixDate("F-200001")).toBe(`F-${yyyy}${mm}`);
  });

  it("preserves custom alpha prefix", () => {
