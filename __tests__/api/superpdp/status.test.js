...
function makeReq({
  method = "GET",
  url = "http://localhost/api/superpdp/status",
  body,
  headers = {},
} = {}) {
...
const { GET } = await import("@/app/api/superpdp/status/route");
...
describe("GET /api/superpdp/status (disabled)", () => {
  it("returns 503 with disabled=true, isConnected=false and a French SuperPDP error", async () => {
