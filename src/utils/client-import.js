// xlsx (~800 KB) est chargé dynamiquement dans parseExcelRaw pour rester
// hors du chunk des pages clients/catalogues.
...
export function validateImportFile(file) {
  ...
  if (!ACCEPTED_EXTENSIONS.includes(ext)) {
    return {
      valid: false,
      error: "Format non supporté. Utilisez CSV, XLS ou XLSX.",
    };
  }
...
export async function parseExcelRaw(file) {
...
export const CLIENT_FIELD_DEFINITIONS = [
  // Général
  {
    ...
    required: false,
    group: "general",
    aliases: ["type", "type de client", "client type", "catégorie", "category"],
  },
  {
    key: "name",
  ...
  },
  {
    ...
    required: false,
    group: "general",
    aliases: ["prénom", "prenom", "first name", "firstname"],
  },
  {
    key: "lastName",
    ...
    required: false,
    group: "general",
    aliases: ["nom de famille", "last name", "lastname", "nom famille"],
  },
  {
    key: "email",
    label: "Email",
  ...
  // Adresse
  {
    ...
    required: false,
    group: "address",
    aliases: ["adresse", "address", "rue", "street", "voie", "adresse postale"],
  },
  {
    key: "city",
    label: "Ville",
  ...
  },
  {
    ...
    required: false,
    group: "address",
    aliases: ["code postal", "postal code", "zip", "zip code", "cp", "code"],
  },
  {
    key: "country",
    label: "Pays",
  ...
  // Légal
  {
    ...
    required: false,
    group: "legal",
    aliases: ["siret", "siren", "n° siret", "numéro siret", "numero siret"],
  },
  {
    key: "vatNumber",
    label: "N° TVA",
  ...
  },
  {
    ...
    required: false,
    group: "legal",
    aliases: ["international", "étranger", "etranger", "hors france"],
  },
];
...
function parseClientType(value) {
  ...
  if (["particulier", "individual", "individu", "personne"].includes(v))
    return "INDIVIDUAL";
  return "COMPANY";
}
...
export function transformRowToClient(
  row,
  headers,
  mapping,
  customFieldMappings = [],
  ...
  if (
    Object.keys(client.address).length === 1 &&
...
export function validateClientRow(clientData, rowIndex) {
  ...
  if (
    ...
    errors.push(
      `Ligne ${rowIndex + 1} : Email invalide "${clientData.email}".`,
    );
  }
  ...
  if (clientData.type === "COMPANY") {
    if (!clientData.siret || clientData.siret.trim() === "") {
      ...
      !clientData.isInternational &&
      !/^(\d{9}|\d{14})$/.test(clientData.siret.replace(/\s/g, ""))
    ) {
      errors.push(
        `Ligne ${rowIndex + 1} : SIRET invalide "${clientData.siret}" (9 ou 14 chiffres attendus).`,
      );
    }
  }
  ...
  clients.forEach((client, index) => {
    ...
    if (emailCounts[email] !== undefined) {
      duplicates.push({
        rowIndex: index,
        email,
        firstOccurrence: emailCounts[email],
      });
    } else {
...
export function downloadErrorsCSV(errors) {
  const lines = [
    "Ligne;Erreur",
    ...errors.map((e) => `"${e.row ?? ""}";"${e.message ?? e}"`),
  ];
  const BOM = "\uFEFF";
