...
export async function searchCompanies(query, limit = 10) {
  ...
  const processAndCacheResults = (results) => {
    const formattedResults = results.map((company) => ({
      ...
      vatNumber: company.siren
        ? `FR${((12 + 3 * (parseInt(company.siren) % 97)) % 97).toString().padStart(2, "0")}${company.siren}`
        : "",
  ...
  try {
    ...
    console.error(
      "Erreur lors de la recherche directe, tentative avec le fallback:",
      error,
    );
...
export function parseAddress(fullAddress, postalCode = "", city = "") {
  ...
  if (postalCode && city) {
    ...
    const postalCityPattern = new RegExp(
      `\\s*${postalCode}\\s*${city}\\s*$`,
      "i",
    );
