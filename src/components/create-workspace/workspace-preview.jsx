...
function SkeletonBar({ className = "", style }) {
  return <div className={`bg-border rounded-full ${className}`} style={style} />;
}

function NavItem({ icon: Icon, width = "w-14", extra }) {
  return (
    <div className="flex items-center gap-2.5 py-[5px] px-3">
      <Icon className="size-[15px] text-muted-foreground shrink-0" />
...
function TableRow({ nameWidth = "w-20" }) {
  return (
    <div className="flex items-center border-b border-border h-[34px]">
      <div className="flex items-center gap-3 w-[220px] px-3 shrink-0">
        <div className="size-[14px] rounded border border-border shrink-0" />
...
export function WorkspacePreview({ step = 1, isNameFocused = false, companyName = "", members = [], selectedPlan = null, logoUrl = null }) {
  ...
  return (
    <div className="relative h-full pt-10 pl-4 overflow-hidden">
      ...
      <div
        className="bg-card rounded-2xl shadow-[0_4px_40px_rgba(0,0,0,0.08)] dark:shadow-[0_4px_40px_rgba(0,0,0,0.3)] origin-top-left transition-all duration-[600ms] ease-[cubic-bezier(0.4,0,0.2,1)]"
      ...
      >
        <div className="flex h-full">
          ...
          {/* SIDEBAR                          */}
          {/* ================================ */}
          <div className="w-[195px] border-r border-border shrink-0 flex flex-col bg-muted rounded-l-2xl">

            {/* ZONE A — Header (highlight on focus) */}
            <div
              className={`px-4 pt-4 pb-9 transition-all duration-300 ${
                isNameFocused && step === 1
                  ? "shadow-[0_0_0_9.5px_rgba(90,80,255,0.2),inset_0_0_0_1.5px_#5A50FF] rounded-2xl relative z-10 bg-card"
                  : "rounded-tl-2xl"
              }`}
            >
              {/* Identity row */}
              <div className="flex items-center gap-2 mb-3">
                <div className="size-7 rounded-md bg-border flex items-center justify-center shrink-0 overflow-hidden">
                  {logoUrl ? (
                    <img src={logoUrl} alt="" className="size-full object-cover" />
                  ) : (
                    <span className="text-muted-foreground text-xs font-semibold">{firstLetter}</span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-1.5 bg-card border border-border rounded-lg px-2 py-1.5">
                <div className="size-5 rounded border border-border flex items-center justify-center shrink-0">
                  <span className="text-[9px] text-muted-foreground font-medium">K</span>
                </div>
                <SkeletonBar className="h-[10px] flex-1" />
                <span className="text-[8px] text-muted-foreground/60 bg-card border border-border rounded px-1 py-0.5 shrink-0">⌘K</span>
                <Search className="size-3.5 text-muted-foreground shrink-0" />
                <span className="text-[8px] text-muted-foreground/60 bg-card border border-border rounded px-1 py-0.5 shrink-0">/</span>
              </div>
            </div>
            ...
            <div className="py-2 space-y-1">
              ...
              {/* Item with chevron */}
              <div className="flex items-center gap-2.5 py-[5px] px-3">
                <Play className="size-[15px] text-muted-foreground shrink-0" />
              ...
              {/* Expandable */}
              <div className="flex items-center gap-2.5 py-[5px] px-3">
                <ChevronDown className="size-[15px] text-muted-foreground shrink-0" />
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex items-center justify-between px-4 pt-4 pb-4">
              ...
              <div className="flex items-center gap-2">
                {step === 3 && filledEmails.length > 0 && (
                  <div className="flex -space-x-2 mr-1">
                    {filledEmails.map((email, i) => (
                      <div
                        key={i}
                        className="size-6 rounded-full flex items-center justify-center text-white text-[9px] font-semibold border-2 border-card"
                ...
                <div className="size-6 rounded-full bg-gradient-to-b from-[#d0d0d0] to-[#b0b0b0] border-2 border-card" />
            ...
            <div className="flex items-center gap-2 border-b border-border px-4 pb-3">
              <div className="flex items-center gap-2 bg-muted rounded-lg px-3 py-1.5" style={{ outline: "1px dashed rgba(128,128,128,0.3)", outlineOffset: "-1px" }}>
            ...
            <div className="relative flex-1">
              ...
              <div className="absolute top-0 bottom-0 left-[220px] w-px bg-accent" />
              <div className="absolute top-0 bottom-0 w-px bg-accent" style={{ left: "calc(220px + (100% - 220px) / 3)" }} />
              <div className="absolute top-0 bottom-0 w-px bg-accent" style={{ left: "calc(220px + (100% - 220px) * 2 / 3)" }} />
              ...
              {step === 3 && filledEmails.length > 0 ? (
                <>
                  {filledEmails.flatMap((email, i) => {
                    ...
                    return [
                      <div key={i} className="flex items-stretch border-b border-border h-[34px]">
                        <div className="flex items-center gap-3 w-[220px] px-3 shrink-0">
                          <div className="size-[14px] rounded border border-border shrink-0" />
                          <div className="size-[18px] rounded-full bg-border shrink-0" />
                          <SkeletonBar className="h-[10px] w-16" />
                      ...
                      <div key={`spacer-${i}`} className="border-b border-border h-[34px]" />,
                    ];
                  })}
                  {Array.from({ length: 10 }).map((_, i) => (
                    <div key={`empty-${i}`} className="border-b border-border h-[34px]" />
                  ))}
                </>
              ) : (
            ...
            {/* Footer */}
            <div className="flex items-center gap-6 px-3 py-2 border-t border-border mt-auto">
              <SkeletonBar className="h-[10px] w-12" />
              <SkeletonBar className="h-[10px] w-12" />
              <SkeletonBar className="h-[10px] w-12" />
              <SkeletonBar className="h-[10px] w-12" />
      ...
      {step === 2 && selectedPlan && (() => {
        ...
        return (
          <div
            key={selectedPlan}
            className="absolute w-[380px] bg-card rounded-3xl p-5 shadow-[0_0_0_1.5px_#5A50FF,0_0_0_9.5px_rgba(90,80,255,0.2),0_8px_30px_rgba(0,0,0,0.08)] dark:shadow-[0_0_0_1.5px_#5A50FF,0_0_0_9.5px_rgba(90,80,255,0.2),0_8px_30px_rgba(0,0,0,0.3)] z-20"
            style={{
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2">
              {plan.features.map((feature) => (
                <div key={feature} className="flex items-center gap-2">
                  <div className="size-5 rounded-md bg-accent flex items-center justify-center shrink-0">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-muted to-transparent pointer-events-none" />
    </div>
  );
}
