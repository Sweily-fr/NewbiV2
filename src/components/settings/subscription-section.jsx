...
import { LoaderCircle, Check, AlertTriangle, Users, X } from "lucide-react";
...
export function SubscriptionSection({
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (data.blockedState) {
          ...
          toast.error(
            data.message ||
              "Veuillez retirer les sièges supplémentaires d'abord",
          );
        } else {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (data.requireNewPreview) {
          toast.error(
            data.message || "Les données ont changé, veuillez recharger",
          );
          ...
          toast.error(
            data.message || "Retirez d'abord les sièges supplémentaires",
          );
        } else {
          toast.error(
  ...
  const currentPlanConfig = PLANS_CONFIG.find(
    (p) => p.key === subscription?.plan,
  );
  const currentPlanIndex = PLANS_CONFIG.findIndex(
    (p) => p.key === subscription?.plan,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {isActive() && subscription && currentPlanConfig && (
        <div className="rounded-xl border border-gray-200 dark:border-[#2c2c2c] p-3">
          <div className="flex items-center gap-3 mb-2">
            <Badge
              variant="secondary"
              className="text-xs font-normal bg-gray-100 dark:bg-[#1a1a1a] text-foreground rounded-md border border-gray-200 dark:border-[#2c2c2c]"
            >
              Actuel
            </Badge>
      ...
      {/* Cancel button — hidden for trial app users (no Stripe subscription to cancel yet) */}
      {isActive() &&
        subscription &&
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={
                isLoading || cancelConfirmText.toLowerCase() !== "allow"
              }
          ...
          {isLoadingPreview ? (
            ...
            <>
              ...
              <div className="px-6 pb-6 space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      >
                        {planChangePreview.proration.prorationAmount >= 0
                          ...
                          : ""}
                        {formatPrice(
                          planChangePreview.proration.prorationAmount,
                        )}{" "}
                        €
                      </span>
                    </div>
                      ...
                      <p className="text-sm">
                        {planChangePreview.change.memberCheck
                          ...
                          <>
                            ...
                            invitation(s) en attente, soit{" "}
                            {
                              planChangePreview.change.memberCheck
                                .totalAfterPending
                            }{" "}
                            au total. Le plan est limité à{" "}
                            {planChangePreview.change.memberCheck.newLimit}.
                            Retirez des membres ou annulez des invitations
                            d'abord.
                          </>
                        ) : (
                          <>
                            ...
                            membres mais ce plan est limité à{" "}
                            {planChangePreview.change.memberCheck.newLimit}.
                            Retirez{" "}
                            {
                              planChangePreview.change.memberCheck
                ...
                {planChangePreview.change.paidSeats > 0 &&
                  planChangePreview.change.isDowngrade && (
                    <Callout type="warning" noMargin>
                      <p className="text-sm">
                        Vous avez {planChangePreview.change.paidSeats} siège(s)
                        supplémentaire(s) payant(s). Retirez-les dans la section
                        "Espaces" avant de changer de plan.
                      </p>
                    </Callout>
                  )}
              </div>
