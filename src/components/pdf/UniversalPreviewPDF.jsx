...
const UniversalPreviewPDF = ({
  ...
  const calculateTotals = (items = []) => {
    ...
    // Pour les factures de situation, utiliser 0 par défaut si progressPercentage n'est pas défini
    ...
    items.forEach((item) => {
      const quantity = parseFloat(item.quantity) || 0;
      const unitPrice = parseFloat(item.unitPrice) || 0;
      // Pour les factures de situation, 0% par défaut; sinon 100%
    ...
    items.forEach((item) => {
      ...
      // Auto-liquidation : TVA = 0 si isReverseCharge = true
      const itemTax =
        data.isReverseCharge || vatRate === 0
          ? 0
    ...
    if (shippingData?.billShipping && shippingData?.shippingAmountHT > 0) {
      ...
      // Auto-liquidation : TVA = 0 si isReverseCharge = true
      shippingTax =
        data.isReverseCharge || shippingVatRate === 0
          ? 0
      ...
      >
        ...
        >
          ...
          <div className="mb-6" data-pdf-section="items">
            {(() => {
              // Pour les factures de situation, toujours afficher les colonnes d'avancement
              const isSituationInvoice = data.invoiceType === "situation";
              // Pour les autres types, vérifier si au moins un article a un avancement différent de 0% ou 100%
              ...
              // Calculer l'avancement cumulé des factures précédentes par article (basé sur l'index)
              // Pour les factures de situation, on additionne les avancements des factures précédentes
              const getPreviousCumulativeProgress = (itemIndex) => {
                if (
                  !previousSituationInvoices ||
                  previousSituationInvoices.length === 0
                ) {
                  return 0;
                }
                ...
                <table className="w-full border-collapse text-xs border-b border-[#CCCCCC]">
                  ...
                  >
                    <tr
                      style={{
                        backgroundColor:
                          data.appearance?.headerBgColor || "#000000",
                      }}
                    >
                      <th
                      ...
                      <th
                        ...
                        style={{
                          color: data.appearance?.headerTextColor || "#FFFFFF",
                          width: isSituationInvoice ? "20%" : "20%",
                        }}
                      >
                        Total HT
                      </th>
                  ...
                  <tbody className="text-[10px]" data-pdf-items-body>
                    {data.items && data.items.length > 0 ? (
                      data.items.map((item, index) => (
                        ...
                        >
                          ...
                          {isSituationInvoice ? (
                            <>
                              {/* Colonne Avancement (pourcentage de cette facture) */}
                              <td
                              ...
                              {/* Colonne Avancement cumulé (avancement précédent + avancement actuel) */}
                              <td
                                className="pt-3 pb-3 px-2 text-center dark:text-[#0A0A0A] align-top"
                                style={{ width: "14%" }}
                              >
                                {(
                                  getPreviousCumulativeProgress(index) +
                          ...
                          >
                            {(() => {
                              // Pour les factures de situation, utiliser 0 si progressPercentage n'est pas défini ou est 0
                              // Pour les autres types, utiliser 100 par défaut
                              const progress = isSituationInvoice
                                ? (item.progressPercentage ?? 0)
                                : (item.progressPercentage ?? 100);
                    ...
                    {(() => {
                      ...
                      return (
                        <tr
                          className="border-b border-[#CCCCCC] no-break"
                          data-no-break="true"
                          style={{
                            pageBreakInside: "avoid",
                            breakInside: "avoid",
            ...
            <div className="w-72 space-y-1 text-xs">
              ...
              {(() => {
                ...
                if (escompteValue > 0) {
                  ...
                  const escompteAmount = (totalHT * escompteValue) / 100;
                  const htAfterEscompte = totalHT - escompteAmount;
                  const tvaAfterEscompte = data.isReverseCharge
                    ? 0
                    : (htAfterEscompte / totalHT) * totalTax;
          ...
          {data.isReverseCharge && (
            <div className="mb-4 text-[10px] pt-2">
              <p className="text-gray-700 dark:text-[#0A0A0A] text-[10px]">
                <span className="font-medium">Autoliquidation :</span> TVA non
                applicable, article 283 du CGI
              </p>
            </div>
          )}
                ...
                <tbody>
                  ...
                  {previousSituationInvoices.map((invoice, index) => {
                    ...
                    const formatDate = (dateStr, fallbackDateStr) => {
                      const dateToUse =
                        dateStr || fallbackDateStr || invoice.updatedAt;
                      if (!dateToUse) return "-";
                    ...
                    // Calculer le montant réel TTC en tenant compte du progressPercentage, remises, escompte, retenue et auto-liquidation
                    const calculateInvoiceTotalTTC = () => {
                      ...
                      // Étape 1: Calculer le total HT des articles
                      invoice.items.forEach((item) => {
                        ...
                        // TVA (0 si auto-liquidation)
                        if (!invoice.isReverseCharge && vatRate > 0) {
                          totalTVA += itemHT * (vatRate / 100);
                        }
                      });

                      // Étape 2: Appliquer la remise globale
                      const globalDiscount = parseFloat(invoice.discount) || 0;
                      if (globalDiscount > 0) {
                        const originalTotalHT = totalHT;
                        if (
                          invoice.discountType?.toUpperCase() === "PERCENTAGE"
                        ) {
                          totalHT =
                      ...
                      >
                        <td className="py-3 px-2 dark:text-[#0A0A0A]">
                          <div className="font-medium text-[11px]">
                            ...
                            {invoice.prefix ? `${invoice.prefix}-` : "F-"}
                            {invoice.number}
                          </div>
                  ...
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="py-3 px-2 text-right dark:text-[#0A0A0A] text-[11px]">
                      {formatCurrency(
                        (() => {
                          // Calculer le total TTC de la facture actuelle avec escompte, retenue et auto-liquidation
                          let totalHT = 0;
                          let totalTVA = 0;
                          ...
                          data.items?.forEach((item) => {
                            ...
                            let itemTotal =
                              quantity * unitPrice * (progress / 100);
                            ...
                            if (!data.isReverseCharge && vatRate > 0) {
                              totalTVA += itemTotal * (vatRate / 100);
                            }
                          });
                          ...
                          if (globalDiscount > 0) {
                            const originalTotalHT = totalHT;
                            if (
                              data.discountType?.toUpperCase() === "PERCENTAGE"
                            ) {
                              totalHT =
                                totalHT *
