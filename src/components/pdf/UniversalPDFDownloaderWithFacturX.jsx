...
class PDFLayoutAnalyzer {
  constructor(element, pixelsPerMM) {
    ...
    this.availableHeight = this.pageHeightPx - this.margins.top - this.margins.bottom;
  }
  ...
  logAnalysis(sections) {
    ...
    console.log(`  ├─ Header: ${sections.header ? '✓' : '✗'} ${sections.header ? `(${(sections.header.height / this.pixelsPerMM).toFixed(1)}mm)` : ''}`);
    console.log(`  ├─ Info client: ${sections.info ? '✓' : '✗'} ${sections.info ? `(${(sections.info.height / this.pixelsPerMM).toFixed(1)}mm)` : ''}`);
    console.log(`  ├─ Table header: ${sections.tableHeader ? '✓' : '✗'} ${sections.tableHeader ? `(${(sections.tableHeader.height / this.pixelsPerMM).toFixed(1)}mm)` : ''}`);
    console.log(`  ├─ Articles: ${sections.items.length} items`);
    console.log(`  ├─ Totaux: ${sections.totals.length} lignes`);
    console.log(`  └─ Footer: ${sections.footer ? '✓' : '✗'} ${sections.footer ? `(${(sections.footer.height / this.pixelsPerMM).toFixed(1)}mm)` : ''}`);
  }
  ...
  calculatePages(analysis) {
    ...
    console.log(`${'='.repeat(60)}`);
    console.log(`Hauteur totale document: ${(totalHeight / this.pixelsPerMM).toFixed(0)}mm`);
    console.log(`Hauteur page A4: ${this.A4_HEIGHT_MM}mm`);
    console.log(`Hauteur utilisable: ${(this.availableHeight / this.pixelsPerMM).toFixed(0)}mm`);
    console.log(`Hauteur footer: ${(footerHeight / this.pixelsPerMM).toFixed(1)}mm`);

    // PAGE 1 : Header + Info + début items + Footer
    console.log(`\n📄 PAGE 1 (première page)`);
    
    // Réserver le footer sur CHAQUE page (il sera répété)
    let page1UsableHeight = this.availableHeight - headerHeight - infoHeight - footerHeight;
    if (sections.tableHeader) {
      page1UsableHeight -= tableHeaderHeight;
    }

    console.log(`  Réservé header: ${(headerHeight / this.pixelsPerMM).toFixed(1)}mm`);
    console.log(`  Réservé info: ${(infoHeight / this.pixelsPerMM).toFixed(1)}mm`);
    console.log(`  Réservé table header: ${(tableHeaderHeight / this.pixelsPerMM).toFixed(1)}mm`);
    console.log(`  Réservé footer: ${(footerHeight / this.pixelsPerMM).toFixed(1)}mm`);
    console.log(`  ➜ Espace items: ${(page1UsableHeight / this.pixelsPerMM).toFixed(1)}mm`);

    const page1StartY = 0;
    const page1ContentStart = headerHeight + infoHeight + (sections.tableHeader ? tableHeaderHeight : 0);
    let page1EndY = page1ContentStart + page1UsableHeight;

    // Trouver les items de la page 1
    const { adjustedEndY: page1AdjustedEnd, itemsIncluded: page1Items } = this.findItemsForPage(
      sections.items,
      page1ContentStart,
      page1EndY,
      1
    ...
    console.log(`  Fin ajustée: ${(page1AdjustedEnd / this.pixelsPerMM).toFixed(1)}mm`);

    currentY = page1AdjustedEnd;

    // PAGES SUIVANTES
    const lastItemIndex = page1Items.length > 0 ? 
      Math.max(...page1Items.map(i => i.itemIndex)) : -1;
    
    const remainingItems = sections.items.filter(item => 
      item.itemIndex !== null && item.itemIndex > lastItemIndex
    );

    let itemsProcessed = page1Items.length;

    while (itemsProcessed < sections.items.length || currentY < totalHeight - footerHeight) {
      pageNumber++;
      console.log(`\n📄 PAGE ${pageNumber}`);

      // Réserver le footer sur TOUTES les pages
      const pageUsableHeight = this.availableHeight - 
        (sections.tableHeader ? tableHeaderHeight : 0) - 
        footerHeight;

      const pageStartY = currentY;
      const pageContentStart = currentY + (sections.tableHeader ? tableHeaderHeight : 0);
      let pageEndY = pageContentStart + pageUsableHeight;

      console.log(`  Espace disponible: ${(pageUsableHeight / this.pixelsPerMM).toFixed(1)}mm`);
      console.log(`  Réservé footer: ${(footerHeight / this.pixelsPerMM).toFixed(1)}mm`);

      // Items restants pour cette page
      const itemsForThisPage = sections.items.filter(item =>
        item.itemIndex !== null &&
      ...
      if (itemsForThisPage.length === 0) {
        console.log(`  ℹ️ Plus d'items, création de la dernière page avec totaux + footer`);
        
        // Dernière page : totaux + footer
        const totalsHeight = sections.totals.reduce((sum, t) => sum + t.height, 0);
      ...
      const { adjustedEndY: pageAdjustedEnd, itemsIncluded: pageItems } = this.findItemsForPage(
        itemsForThisPage,
        pageContentStart,
        pageEndY,
        pageNumber
      ...
      const isLastPage = itemsProcessed + pageItems.length >= sections.items.length;
      
      // Calculer l'espace nécessaire pour les totaux
      const totalsHeight = sections.totals.reduce((sum, t) => sum + t.height, 0);
      const spaceAfterItems = pageEndY - pageAdjustedEnd;
      const canIncludeTotals = isLastPage && spaceAfterItems >= totalsHeight + this.margins.safe;

      pages.push({
        pageNumber,
        startY: pageStartY,
        endY: pageAdjustedEnd + footerHeight + (canIncludeTotals ? totalsHeight : 0) + this.margins.safe,
      ...
      if (canIncludeTotals) {
        ...
        console.log(`  ⚠️ Dernière page mais pas assez de place pour les totaux`);
      ...
      if (isLastPage) {
        ...
        pages.push({
          ...
          endY: finalPageStartY + totalsHeight + footerHeight + this.margins.safe,
      ...
      if (item.top < endY) {
        ...
        if (item.bottom > endY) {
          if (!item.canBreak) {
            ...
            console.log(`  ⚠️ Item ${item.itemIndex} serait coupé (${(item.height / this.pixelsPerMM).toFixed(1)}mm)`);
        ...
        if (item.keepWithNext) {
          const nextItem = items.find(i => i.itemIndex === item.itemIndex + 1);
          if (nextItem && nextItem.bottom > endY) {
            console.log(`  🔗 Item ${item.itemIndex} doit rester avec le suivant`);
            adjustedEndY = item.top - this.margins.itemSpacing;
            itemsIncluded.pop();
            break;
          }
  ...
  const handlePDFDownload = async () => {
    ...
    try {
      ...
      const sections = {
        ...
        headerNotes: await captureSection('[data-pdf-section="header-notes"]', 'Note d\'en-tête'),
        marketAmount: await captureSection('[data-pdf-section="market-amount"]', 'Montant du marché'),
        tableHeader: await captureSection('[data-pdf-table-header]', 'Table header'),
        items: [], // On capturera les items individuellement
        totals: await captureSection('[data-pdf-section="totals"]', 'Totaux'),
        vatExemption: await captureSection('[data-pdf-section="vat-exemption"]', 'Exonération TVA'),
      ...
      console.log('\n📸 Capture des items...');
      const itemElements = componentRef.current.querySelectorAll('[data-pdf-item]');
      for (let i = 0; i < itemElements.length; i++) {
        const itemCapture = await captureSection(`[data-pdf-item][data-item-index="${i}"]`, `Item ${i + 1}`);
        if (itemCapture) {
      ...
      const referenceImg = sections.header?.img || sections.info?.img || sections.items[0]?.img;
      if (!referenceImg) {
      ...
      console.log(`Hauteur disponible par page: ${AVAILABLE_HEIGHT.toFixed(1)}mm (pagination: ${PAGINATION_HEIGHT}mm)`);
      ...
      currentPage.sections.push({ type: 'header', data: sections.header, height: heights.header, spacing: SECTION_SPACING });
      currentPage.currentHeight += heights.header + SECTION_SPACING;
      
      if (sections.info) {
        currentPage.sections.push({ type: 'info', data: sections.info, height: heights.info, spacing: SECTION_SPACING * 2 }); // Double espacement avant les notes
        currentPage.currentHeight += heights.info + (SECTION_SPACING * 2);
      }
      
      if (sections.headerNotes) {
        currentPage.sections.push({ type: 'headerNotes', data: sections.headerNotes, height: heights.headerNotes, spacing: SECTION_SPACING });
        currentPage.currentHeight += heights.headerNotes + SECTION_SPACING;
      }
      
      if (sections.marketAmount) {
        currentPage.sections.push({ type: 'marketAmount', data: sections.marketAmount, height: heights.marketAmount, spacing: SECTION_SPACING });
        currentPage.currentHeight += heights.marketAmount + SECTION_SPACING;
      }
      
      if (sections.tableHeader) {
        currentPage.sections.push({ type: 'tableHeader', data: sections.tableHeader, height: heights.tableHeader, spacing: 0 });
      ...
      console.log(`   Hauteur disponible pour items: ${AVAILABLE_HEIGHT.toFixed(1)}mm (sans réservation footer)`);
      
      for (let i = 0; i < sections.items.length; i++) {
        const itemHeight = heights.items[i];
        const itemSpacing = i < sections.items.length - 1 ? 0.5 : SECTION_SPACING;
        const totalHeight = itemHeight + itemSpacing;
        
        console.log(`  Item ${i + 1}: ${itemHeight.toFixed(1)}mm (page actuelle: ${currentPage.currentHeight.toFixed(1)}mm)`);
        ...
        if (currentPage.currentHeight + totalHeight > AVAILABLE_HEIGHT) {
          ...
          if (sections.tableHeader) {
            ...
            currentPage.currentHeight += heights.tableHeader;
            console.log(`    + Table header répété (${heights.tableHeader.toFixed(1)}mm)`);
          }
        }
        ...
        console.log(`    ✓ Item ${i + 1} ajouté (hauteur totale page: ${currentPage.currentHeight.toFixed(1)}mm)`);
      }
      ...
      if (sections.totals) totalFinalSectionsHeight += heights.totals + SECTION_SPACING;
      if (sections.vatExemption) totalFinalSectionsHeight += heights.vatExemption + SECTION_SPACING;
      if (sections.terms) totalFinalSectionsHeight += heights.terms + SECTION_SPACING;
      
      // Vérifier si TOUT (sections finales + footer) rentre sur la page actuelle
      const everythingFitsOnCurrentPage = (currentPage.currentHeight + totalFinalSectionsHeight + heights.footer) <= AVAILABLE_HEIGHT;
      
      if (everythingFitsOnCurrentPage) {
        console.log(`   ✅ Toutes les sections finales + footer rentrent sur cette page`);
        console.log(`   ⚠️ Réservation de l'espace du footer (${heights.footer.toFixed(1)}mm)`);
      } else {
        console.log(`   ⚠️ Les sections finales ne rentrent pas toutes avec le footer`);
        console.log(`   ➜ Utilisation de toute la hauteur disponible (pas de réservation footer)`);
      }
      ...
      if (sections.totals) {
        ...
        if (spaceCheck > AVAILABLE_HEIGHT) {
          ...
          const newPageCheck = totalFinalSectionsHeight + heights.footer <= AVAILABLE_HEIGHT;
          if (newPageCheck) {
            console.log(`   ✅ Sur cette nouvelle page, tout rentre avec le footer`);
          }
        }
      ...
      const breakableSections = [
        { type: 'vatExemption', data: sections.vatExemption, height: heights.vatExemption },
      ...
      for (const section of breakableSections) {
        ...
        const remainingFinalSectionsHeight = breakableSections
          .filter(s => s.type === section.type || breakableSections.indexOf(s) > breakableSections.indexOf(section))
          .reduce((sum, s) => sum + s.height + SECTION_SPACING, 0);
        
        const allFitsNow = (currentPage.currentHeight + remainingFinalSectionsHeight + heights.footer) <= AVAILABLE_HEIGHT;
        ...
        console.log(`  ${section.type}: ${section.height.toFixed(1)}mm (espace restant: ${spaceRemaining.toFixed(1)}mm)`);
        ...
        if (section.height <= spaceRemaining) {
          ...
          console.log(`    ⚠️ Section trop longue, sera coupée sur plusieurs pages`);
          
          // Ajouter la partie qui rentre sur la page actuelle
          if (spaceRemaining > 20) { // Au moins 20mm pour que ça vaille le coup
            currentPage.sections.push({ 
              type: section.type, 
              data: section.data, 
            ...
            console.log(`    ✓ Partie 1 ajoutée (${(spaceRemaining - SECTION_SPACING).toFixed(1)}mm)`);
            ...
            const remainingHeight = section.height - (spaceRemaining - SECTION_SPACING);
            currentPage.sections.push({ 
              type: section.type, 
              data: section.data, 
            ...
            console.log(`    ✓ Partie 2 ajoutée (${remainingHeight.toFixed(1)}mm)`);
          } else {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        for (const section of page.sections) {
          ...
          if (section.isPartial) {
            console.log(`  + ${section.type} (PARTIEL: ${section.height.toFixed(1)}mm de ${section.partialStart.toFixed(1)}mm à ${(section.partialStart + section.partialHeight).toFixed(1)}mm)`);
          } else {
            console.log(`  + ${section.type} (${section.height.toFixed(1)}mm + ${section.spacing.toFixed(1)}mm spacing)`);
          }
          ...
          if (section.isPartial) {
            ...
            ctx.drawImage(
              img,
              0, startPixels,           // Source x, y
              img.width, heightPixels,  // Source width, height
              0, 0,                     // Dest x, y
              img.width, heightPixels   // Dest width, height
            );
        ...
        if (sections.footer && i === pages.length - 1) {
          ...
          const footerYPosition = Math.floor((paginationY - heights.footer) * 100) / 100;
          ...
          console.log(`  + Footer collé à la pagination (${footerYPosition.toFixed(2)}mm)`);
        }
      ...
      const fileName = filename || (prefix ? `${documentType}_${prefix}-${number}.pdf` : `${documentType}_${number}.pdf`);
      ...
      if (canUseFacturX) {
        ...
        if (validation.isValid) {
          ...
          console.warn('⚠️ Validation Factur-X échouée:', validation.errors.slice(0, 3));
