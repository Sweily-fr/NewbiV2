...
export const useCrmEmailAutomationLogs = (
  workspaceId,
  automationId,
  limit = 50,
) => {
...
export const useCreateCrmEmailAutomation = () => {
  const [createMutation, { loading, error }] = useMutation(
    CREATE_CRM_EMAIL_AUTOMATION,
    {
      onError: (error) => {
        toast.error(
          error.message || "Erreur lors de la création de l'automatisation",
        );
      },
    },
  );
  ...
  const createAutomation = async (workspaceId, input) => {
    try {
      const { data } = await createMutation({
        ...
        refetchQueries: [
          { query: GET_CRM_EMAIL_AUTOMATIONS, variables: { workspaceId } },
        ],
      });
      return data.createCrmEmailAutomation;
    } catch (err) {
      console.error(
        "Erreur lors de la création de l'automatisation email:",
        err,
      );
      throw err;
    }
...
export const useUpdateCrmEmailAutomation = () => {
  const [updateMutation, { loading, error }] = useMutation(
    UPDATE_CRM_EMAIL_AUTOMATION,
    {
      onError: (error) => {
        toast.error(
          error.message || "Erreur lors de la modification de l'automatisation",
        );
      },
    },
  );
  ...
  const updateAutomation = async (workspaceId, id, input) => {
    try {
      ...
      console.error(
        "Erreur lors de la modification de l'automatisation email:",
        err,
      );
      throw err;
    }
...
export const useDeleteCrmEmailAutomation = () => {
  const [deleteMutation, { loading, error }] = useMutation(
    DELETE_CRM_EMAIL_AUTOMATION,
    {
      onError: (error) => {
        toast.error(
          error.message || "Erreur lors de la suppression de l'automatisation",
        );
      },
    },
  );
  ...
  const deleteAutomation = async (workspaceId, id) => {
    try {
      ...
      console.error(
        "Erreur lors de la suppression de l'automatisation email:",
        err,
      );
      throw err;
    }
...
export const useToggleCrmEmailAutomation = () => {
  const [toggleMutation, { loading, error }] = useMutation(
    TOGGLE_CRM_EMAIL_AUTOMATION,
    {
      onError: (error) => {
        toast.error(
          error.message || "Erreur lors du basculement de l'automatisation",
        );
      },
    },
  );
  ...
  const toggleAutomation = async (workspaceId, id) => {
    try {
      ...
      console.error(
        "Erreur lors du basculement de l'automatisation email:",
        err,
      );
      throw err;
    }
