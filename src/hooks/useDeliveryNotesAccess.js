...
// Comptes de test hors domaine (ex. compte local de Dylan)
export const DELIVERY_NOTES_ALLOWED_EMAILS = ["dylan.lobjois@outlook.com"];

export function isDeliveryNotesAllowed(email) {
  if (!email) return false;
  const normalized = String(email).trim().toLowerCase();
  if (DELIVERY_NOTES_ALLOWED_EMAILS.includes(normalized)) return true;
