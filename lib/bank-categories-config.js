...
export const getTransactionCategory = (transaction) => {
  ...
  const rawDescription = (transaction.description || "")
    ...
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/^(cb |vir |prlv |cheque |chq |retrait |dab |tip )/i, "") // Supprimer préfixes bancaires
    .trim();
...
export const aggregateByCategory = (transactions, isIncome = false) => {
  ...
  // Debug: log quelques descriptions pour voir le format
  console.log(
    "🏷️ [aggregateByCategory] Exemples de descriptions:",
    transactions.slice(0, 5).map((t) => t.description)
  );
