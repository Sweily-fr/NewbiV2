...
export const incomeCategories = {
  ...
  // Chiffre d'affaires : catch-all des revenus (factures clients + importées
  // et tout revenu non rattaché à une catégorie spécifique).
...
export const getTransactionCategory = (transaction) => {
  ...
  // Revenu rattaché à une facture client/importée → Chiffre d'affaires.
  // Prioritaire sur l'ID Bridge pour rester cohérent avec le backend.
  ...
  if (isIncome) {
    ...
    return incomeCategories[0]; // Chiffre d'affaires
  } else {
...
// Les transactions doivent être pré-filtrées par signe (positif/négatif) avant l'appel
