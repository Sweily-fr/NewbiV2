...
test.describe("Authentication flows", () => {
  test.describe("Login", () => {
    ...
    test("should show error with invalid credentials", async ({ page }) => {
      ...
      await expect(
        page
          .locator('text="Email ou mot de passe incorrect"')
  ...
  test.describe("Signup", () => {
    ...
    test("should validate required fields on submit", async ({ page }) => {
      ...
      if (await submitButton.isVisible()) {
        ...
        // After invalid submit we expect to remain on the auth route — wait for
        // either a validation message or simply that the URL stays under /auth.
        await page
          .waitForFunction(() => window.location.pathname.includes("/auth"), {
            timeout: 5000,
          })
          .catch(() => {});
